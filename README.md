...
- 🚨 **单日用量超额预警（非强制，自由开关）**：当单日用量计费超过自定义设定限额（默认 300 元，可在桌宠主界面汉堡菜单或设置中心直接开关及修改金额）时，小鲸鱼伴随惊慌 Q 弹动效自动弹出专属告警气泡：**「主人歇歇吧！要..要被玩坏了...」**，防止 API 费用意外超支，内置单日防频繁打扰保护与跨天自动重置；
- ⚙️ **汉堡菜单与系统托盘**：
  - 桌宠悬停右上角汉堡菜单快速调节：缩放（0.6x ~ 2.5x）、音效、音量、用量模式、峰谷文案风格、气泡开关、吸附范围（0~200px）、每轮消耗开关、用量预警开关与自定义预警限额（元）；
  ...
  - 独立设置中心：可视化配置 `DEEPSEEK_API_KEY`、`DEEPSEEK_PLATFORM_TOKEN`、单日用量预警阈值（默认 300 元）与即时测试、开机自启、本地端口。
