...
| 💥 **伤害飘字与受击反馈** | 扣费瞬间头顶弹出游戏风伤害数字：普通 / 重击 / 暴击三级配色，**缓存未命中自动升为暴击**；伴随鲸鱼受击抖动、`COMBO xN` 连击计数与打击音效，深色胶囊**常驻血条**实时下降（越低越红）、峰谷徽章红绿切换，右端恒定避开小鲸鱼、长度 0.6x~1.6x 可调。汉堡菜单「飘字 / 血条长度」随心调节。 |
| 🧪 **测试版面板** | 设置中心内置测试模式：**自定义余额**、单次扣费金额、`💥 普通扣费` / `⚡ 暴击扣费` 两个按钮强制分级，血条随点击实时扣减；用 `--test` 启动参数秒进测试模式且**不污染**日常配置。<br/>**血条可调**：汉堡菜单「血条」一键显隐、「血条长度」0.6x~1.6x，位置**直接按住血条左右拖动**（手型指针提示可拖）。窗口已按最远位移预留宽度，调整长度 / 拖动位置**全程零窗口 resize**（不会一闪一闪），且绝不压到小鲸鱼、绝不拖出屏幕。 |
