...
- 🚨 **单日用量超额预警**：当单日用量计费超过设定阈值（默认 300 元，可在设置中心自由自定义阈值与开关）时，小鲸鱼伴随惊慌 Q 弹动效自动弹出专属告警气泡：**「主人歇歇吧！要..要被玩坏了...」**，防止 API 费用超支，内置单日防频繁弹窗保护与跨天自动重置；
- ⚙️ **汉堡菜单与系统托盘**：
  - 桌宠悬停右上角汉堡菜单快速调节：缩放（0.6x ~ 2.5x）、音效、音量、用量模式、峰谷文案风格、气泡开关、吸附范围（0~200px）、每轮消耗开关、用量预警开关；
