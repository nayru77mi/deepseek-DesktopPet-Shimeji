...
| 🧪 **测试版面板** | 设置中心内置测试模式：**自定义余额**、单次扣费金额、`💥 普通扣费` / `⚡ 暴击扣费` 两个按钮强制分级，血条随点击实时扣减；用 `--test` 启动参数秒进测试模式且**不污染**日常配置。<br/>**血条可调**：汉堡菜单「血条」一键显隐、「血条长度」0.6x~1.6x、「血条位置」左右微调，也可**直接按住血条左右拖动**（窗口自动同步加宽，绝不压到小鲸鱼、绝不拖出窗口）。 |
