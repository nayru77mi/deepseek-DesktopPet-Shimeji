...
| 版本类型 | 文件名示例 | 特点说明 |
| :--- | :--- | :--- |
| **标准安装包（推荐）** | `DeepSeek.Setup.1.2.0.exe` | 运行安装向导，自动生成带高清图标的桌面与开始菜单快捷方式，支持完整卸载。 |
| **绿色便携版** | `DeepSeek.1.2.0.exe` | **无需安装**，直接双击运行，适合快速体验或随身 U 盘携带。 |
...
>
> **🧪 想先看动效？** 设置中心里勾选「测试版面板」，或用 `DeepSeek.1.2.0.exe --test` 启动，即可用自定义余额反复演练普通 / 暴击扣费。
